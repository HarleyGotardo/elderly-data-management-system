...
-- Policy for LGU users to see their own records
CREATE POLICY "LGU users can view their own senior citizens" ON senior_citizens
    FOR SELECT USING (lgu_id = current_setting('app.current_lgu_id')::integer);

-- Policy for LGU users to insert their own records
CREATE POLICY "LGU users can insert their own senior citizens" ON senior_citizens
    FOR INSERT WITH CHECK (lgu_id = current_setting('app.current_lgu_id')::integer);

-- Policy for LGU users to update their own records
CREATE POLICY "LGU users can update their own senior citizens" ON senior_citizens
    FOR UPDATE USING (lgu_id = current_setting('app.current_lgu_id')::integer);

-- Policy for Admin/Super Admin to view all records
CREATE POLICY "Admin users can view all senior citizens" ON senior_citizens
    FOR SELECT USING (
        current_setting('app.current_role') IN ('Admin', 'Super Admin')
    );

-- Policy for Admin/Super Admin to update all records
CREATE POLICY "Admin users can update all senior citizens" ON senior_citizens
    FOR UPDATE USING (
        current_setting('app.current_role') IN ('Admin', 'Super Admin')
    );
