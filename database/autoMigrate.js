const db = require('./config.js');
const bcrypt = require('bcryptjs');

/**
 * Auto-migration script that runs in Electron context
 */
async function runMigrations() {
  console.log('Running auto-migrations...');
  
  try {
    // Check if users table exists and has the correct schema
    const tableInfo = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='users'").get();
    
    if (!tableInfo) {
      // Create users table
      console.log('Creating users table...');
      db.exec(`
        CREATE TABLE users (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          username TEXT UNIQUE NOT NULL,
          password_hash TEXT NOT NULL,
          role TEXT NOT NULL CHECK (role IN ('Client', 'Admin', 'Super Admin')),
          created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
          updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
        );
        
        CREATE INDEX IF NOT EXISTS idx_users_username ON users(username);
        CREATE INDEX IF NOT EXISTS idx_users_role ON users(role);
      `);
      
      // Create default Super Admin user
      const defaultPassword = 'admin123';
      const hashedPassword = bcrypt.hashSync(defaultPassword, 10);
      
      const stmt = db.prepare(`
        INSERT INTO users (username, password_hash, role) 
        VALUES (?, ?, ?)
      `);
      
      stmt.run('admin', hashedPassword, 'Super Admin');
      
      console.log('Users table created with default Super Admin user:');
      console.log('Username: admin');
      console.log('Password: admin123');
    } else {
      // Check if table has the correct columns
      const columns = db.prepare("PRAGMA table_info(users)").all();
      const hasRequiredColumns = columns.some(col => col.name === 'username') &&
                                columns.some(col => col.name === 'password_hash') &&
                                columns.some(col => col.name === 'role');
      
      if (!hasRequiredColumns) {
        console.log('Users table exists but schema is incorrect. Please backup and recreate.');
      } else {
        console.log('Users table already exists with correct schema.');
      }
    }
    
    console.log('Auto-migration completed successfully!');
  } catch (error) {
    console.error('Migration failed:', error);
    throw error;
  }
}

module.exports = { runMigrations };
